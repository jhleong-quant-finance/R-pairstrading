...
cfg_prescreen_data_file = paste0(cfg_pairs_trade_Rdata_path,'prescreen_same_sector_rolling.rda');
cfg_matched_pairs_data_file = paste0(cfg_pairs_trade_Rdata_path,'matched_pairs_same_sector_rolling.rda');
...
if(file.exists(cfg_prescreen_data_file)){
  ...
  for(j in (i+1):num_stock)
  {
    ...
    if(!is.na(sector_ind1[1]) && !is.na(sector_ind2[1])){
